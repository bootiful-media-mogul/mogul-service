...
//@Component
class BinaryMarkdownPreview
        extends MediaTypeMarkdownPreview {
    ...
    public String preview(Attachment payload) {
        this.log.info("previewing {} as a {}",
                payload.managedFile().id(),
                this.mediaTypes.length > 0 ? this.mediaTypes[0].toString() : null);
        return this.managedFileService
                .getPublicUrlForManagedFile(payload.managedFile().id());
    }
}
