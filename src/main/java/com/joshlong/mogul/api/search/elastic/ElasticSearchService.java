...
class ElasticSearchService implements SearchService {
	...
	public Collection<RankedSearchResult> search(String shouldContain, Map<String, Object> metadata) {
		log.info("searching for {} with metadata {}", shouldContain, metadata);
		var nativeQuery = NativeQuery.builder().withQuery(q -> q.bool(b -> {
			// Must match this term
			b.must(m -> m
				.multiMatch(mm -> mm.query(shouldContain).fields("title^2", "description^2").fuzziness("AUTO")));
		...
		results.sort((a, b) -> Double.compare(b.rank(), a.rank()));
...
@org.springframework.data.elasticsearch.annotations.Document(createIndex = false, indexName = "documents")
