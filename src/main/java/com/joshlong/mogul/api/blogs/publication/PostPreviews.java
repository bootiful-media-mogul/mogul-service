...
@Service
@Transactional
class DefaultPostPreviews implements PostPreviews {
    ...
    DefaultPostPreviews(JdbcClient db, ManagedFileService managedFileService, BlogService blogService) {
        this.db = db;
    ...
    public PostPreview createPostPreview(Long mogulId, Long postId) {
        var mf = this.managedFileService.createManagedFile(mogulId, "post-previews",
                Long.toString(postId), 0, MediaType.APPLICATION_OCTET_STREAM, true);
        var gkh = new GeneratedKeyHolder();
        this.db.sql("insert into blog_post_preview (mogul_id, post_id, managed_file_id) values (?, ?, ?)") //
    ...
    public PostPreview getPostPreviewById(Long id) {
        return this.db
                .sql("select * from blog_post_preview where id = ?")
                .param(id)
                .query(this.pp)
                .single();
    ...
    private class PostPreviewRowMapper implements RowMapper<PostPreview> {
        ...
        public PostPreview mapRow(ResultSet rs, int rowNum) throws SQLException {
            return new PostPreview(rs.getLong("id"),
