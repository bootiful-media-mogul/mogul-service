...
import com.joshlong.mogul.api.archives.ArchiveReader;
...
class BlogPostImporter {
	...
	BlogPostImporter(ManagedFileService managedFileService, BlogService blogService, ArchiveReader archiveReader,
	...
	void importBlogPostsFromArchive(long mogulId, long blogId, long managedFileId) throws Exception {
		...
		try (var bufferedInputStream = new BufferedInputStream(resourceForArchive.getInputStream())) {
			...
			this.archiveReader.read(bufferedInputStream, Consumers.readableTextOnly(files::add));
